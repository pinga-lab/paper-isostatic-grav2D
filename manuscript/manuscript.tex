...
The only difference between the simple models I and II is the geometry of basement.
...
In this case, we have used all the constraints.

Define the values of inversion parameters (alphas, S0, upper/lower bounds, Marquardt
parameters). 

true basement and Moho, the gravity disturbance data and the 

Figure~\ref{fig:model-simple-I-no-vinc-isostatic} shows the results obtained for
the simple model I, but without using the isostatic constraint (equation \ref{eq:isostatic-constraint-function}).
...
\begin{center}
	\begin{tabular}{lcc}
		...
		\hline 
		continental crust & 2670 & from beginning until $y_{COT}$ \\
		oceanic crust & 2840 & from $y_{COT}$ until the end \\
		\hline
...
\begin{center}
	\begin{tabular}{lcc}
		...
		\hline 
		continental crust & 2770 & from beginning until $y_{COT}$ \\
		oceanic crust & 2865 & from $y_{COT}$ until the end \\
		\hline
...
\begin{center}
	\begin{tabular}{lcc}
		...
		\hline 
		continental crust & XXXX & from beginning until $y_{COT}$ \\
		oceanic crust & XXXX & from $y_{COT}$ until the end \\
		\hline
...
\plot{model-pelotas-synthetic-data}{width=0.6\textwidth}{
	...
	{Application to synthetic data produced by the volcanic margin model.
	 This model is formed by four layers representing: water, sediments $+$
	 SDR, crust (continental and oceanic) and mantle (Table~\ref{tbl:volcanic-margin-model}).
	 The bottom panel shows the estimated model (gray prisms),
	 the true basement and Moho (dashed white lines), the initial basement
	 and Moho used in the inversion (continuous white lines), as well as the 
	 known depths at basement (white triangles) and Moho (white diamonds).

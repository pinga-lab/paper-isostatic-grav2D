...
juxtaposed cells with constant physical properties. 
...
sedimentary basin.
It is known that these methods suffer from a greater ambiguity in compared with 
...
Among those methods directly estimating the geometry of the surfaces, 
there are some regional and global scale methods in the space domain that impose 
...
Some local-scale methods have also...


\citet{salem-etal2014} presented one of the few local scale methods in space 
domain for estimating the geometry of basement and Moho.


They approximate the subsurface by 2D stacked bodies that have constant density contrasts,
polygonal cross-sections and represent the sediments, crust and mantle. 

The surfaces separating the prisms represent the basement and the Moho.

The depth of the vertices defining these surfaces are the 
parameters to be estimated iteratively.

At each iteration, the depth of the vertices associated with the basement are 
estimated based on the difference between the observed and predicted gravity data,




 Their method estimates the basement
...
is used to update the Moho geometry.



they estimate the moho, remove their gravitational effect and then use the residual 
anomaly to estimate correction at basement

they do not simultaneously estimate basement and Moho relief
 

They impose a perfect isostatic equilibrium according to
the Airy-Heiskanen model \citep[e.g.,][$\,$p. 135]{heiskanen-moritz1967}, 
...
LOCAL SCALE METHODS IMPOSING ISOSTATIC EQUILIBRIUM

\citet{condi-etal1999, ferderer-etal2017, garcia-abdeslem2017, salem2017} presented 
similar local scale methods.

Our method is also substantially different from that presented by García-Abdeslem (2017). 
According to García-Abdeslem (page G52), “An estimate of the crustal thickness for the 
region surrounding the Montage Basin, based upon the flexural isostatic response to 
surface loads, was used to compute an  isostatic regional gravity, which, subtracted 
from Bouguer gravity anomalies, led to the isostatic residual gravity anomaly at Montage 
Basin”. Then, after estimating the crustal thickness explaining the “isostatic 
regional gravity”, the author estimated the depth to the basement at Montage Basin by 
solving a nonlinear inverse problem for explaining the “isostatic residual gravity anomaly”. 
Notice that, different from our method, that presented by García-Abdeslem (2017) does 
not estimate the basement relief and Moho simultaneously.

Finally, there are some similarities between our method and that presented in the 
SEG expanded abstract authored by Ferderer et al. (2017). Our parameterization is 
similar and we also used a concept similar to their “general local isostasy” (GLI). 
According to the GLI, the “crust and mantle densities as well as the depth to the 
top and base of the crust may vary laterally while local isostatic equilibrium is 
maintained”. Their global inversion strategy, which “falls under the category of 
“enumerative or grid search techniques” of Sen and Stoffa (1995)”, uses a “model space 
within which all models obey GLI is defined and a set of trial models which regularly 
span the model space is evaluated”. On the other hand, we formulated a different 
nonlinear inverse problem that uses the Levenberg-Marquardt method for simultaneously 
estimating the geometries of basement and Moho. We present a constraint introducing 
the information that the lithostatic stress must be mostly smooth, except at some 
isolated regions, where it can present abrupt variations.
...
\plot{volcanic-margin-step1}{width=0.6\textwidth}{
...
\plot{volcanic-margin-step2}{width=0.6\textwidth}{
...
\plot{volcanic-margin-step3}{width=0.6\textwidth}{
...
\plot{volcanic-margin-step1-more-inf}{width=0.6\textwidth}{
...
\plot{volcanic-margin-step3-more-inf}{width=0.6\textwidth}{
	{Application to synthetic data. Results obtained in Step 3 by using 
...
\plot{pelotas-step3}{width=0.6\textwidth}{
	...
	inversion (initial guess) and known depths at basement and Moho.

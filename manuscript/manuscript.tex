...
%We approximate the margin by an interpretation model formed by four layers.
...
The acceptability criterion described above to define the values of $\tilde{\mu}_{2}$ and 
$\tilde{\mu}_{3}$ is represented by the decision point $D(\tilde{\mu}_{2}, \tilde{\mu}_{3})$ 
...
This acceptability criterion to define the value of $\tilde{\mu}_{1}$ is
represented by the decision point $D(\tilde{\mu}_{1})$ 
at the flow diagram (Figure~\ref{fig:flow-diagram}).
...
\plot{flow_diagram}{width=0.5\textwidth}{
	...
	explained at subsection \textbf{Computational procedures for solving of the inverse problem}.}
	\label{fig:flow-diagram}
}

%% application to synthetic data

\plot{B-model-rifted-margin-grafics-estimated-model-alphas_X_1_1_2}{width=0.6\textwidth}{
	...
	\label{fig:volc-margem-step1-2112}
}

\plot{B-model-rifted-margin-grafics-estimated-model-alphas_2_1_1_2}{width=0.6\textwidth}{
	{Application to synthetic data. Results obtained at Step 2
	by using $\tilde{\mu}_{0} = 10^{2}$ (equation~\ref{eq:constraint-weights}).
	The remaining informations are the same shown in the caption of
	Figure~\ref{fig:volc-margem-step1}.}
	\label{fig:volc-margem-step2-2112}
}

\plot{B-model-rifted-margin-grafics-estimated-model-alphas_2_1_1_2-sgm_1}{width=0.6\textwidth}{
	...
	The remaining informations are the same shown in the caption of
	Figure~\ref{fig:volc-margem-step1}.}
	\label{fig:volc-margem-step3-2112}
}

\plot{B-model-rifted-margin-grafics-estimated-model-alphas_X_2_1_2}{width=0.6\textwidth}{
	{Application to synthetic data. Results obtained at Step 1. 
		(Bottom panel) Estimated and true surfaces,
		initial basement and Moho used in the inversion (initial guess) and a 
		priori information at some points on basement and Moho (known depths).
		...
		$10^{1}$, $10^{1}$ and $10^{2}$, respectively, $S_{0} = 41$ km and the density 
		contrasts are those shown in Table~\ref{tab:volcanic-margin-model}.}
	\label{fig:volc-margem-step1-3212}
}

\plot{B-model-rifted-margin-grafics-estimated-model-alphas_3_2_1_2}{width=0.6\textwidth}{
	{Application to synthetic data. Results obtained at Step 2
		by using $\tilde{\mu}_{0} = 10^{2}$ (equation~\ref{eq:constraint-weights}).
		The remaining informations are the same shown in the caption of
		Figure~\ref{fig:volc-margem-step1}.}
	\label{fig:volc-margem-step2-3212}
}

\plot{B-model-rifted-margin-grafics-estimated-model-alphas_3_2_1_2-sgm_30}{width=0.6\textwidth}{
	{Application to synthetic data. Results obtained at Step 3 by using 
		the estimated model shown in Figure~\ref{fig:volc-margem-step2} 
		as initial approximation (dashed blue lines) and
		$\sigma = 22$ (equation~\ref{eq:elements-wii}).
		The remaining informations are the same shown in the caption of
		Figure~\ref{fig:volc-margem-step1}.}
	\label{fig:volc-margem-step3-3212}
}

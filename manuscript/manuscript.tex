...
\begin{itemize}
	...
	(equation~\ref{eq:equality-constraint-moho}) containing known basement and Moho depths
...
\begin{enumerate}
	...
	Define an initial approximation $\mathbf{p}_{0}$ satisfying the inequality constraint
	(equation~\ref{eq:inequality-constraint}). Use $\mathbf{p}_{0}$ to compute the Hessian
	...
	$\ell = 1, 2, 3$. Estimate a parameter vector $\mathbf{p}^{(0)}$
	...
	\item [\textbf{(Step 2)}] Use $\mathbf{p}^{(0)}$ as initial approximation. Set $\mathbf{W}$
	(equation~\ref{eq:isostatic-constraint-function}) equal to identity. Set a non-null
	value for $\tilde{\alpha}_{0}$ (equation~\ref{eq:constraint-weights}). 
	Use $\mathbf{p}^{(0)}$ to compute the Hessian
	matrices of the constraining functions $\Psi_{\ell}(\mathbf{p})$, $\ell = 0, 1, 2, 3$
	(equations \ref{eq:isostatic-constraint-function}, \ref{eq:smootheness-contraint},
	\ref{eq:equality-constraint-basement} and \ref{eq:equality-constraint-moho}), and 
	the Hessian matrix of the misfit function
	$\Phi(\mathbf{p})$ (equation~\ref{eq:misfit-function}). Use these Hessian matrices to
	compute $E_{\Phi}$, $E_{\ell}$ and $\alpha_{\ell}$ (equation~\ref{eq:constraint-weights}),
	$\ell = 0, 1, 2, 3$.
	Estimate a parameter vector $\mathbf{p}^{(1)}$ minimizing $\Gamma(\mathbf{p})$ (equation~\ref{eq:goal-function}), subject to the inequality constraint 
	(equation~\ref{eq:inequality-constraint}).
	\item [\textbf{(Step 3)}] Use $\mathbf{p}^{(1)}$ as initial approximation. Compute the 
	diagonal elements $w_{ii}$, $i = 1, \dots N-1$ (equation~\ref{eq:elements-wii}).
	Use $\mathbf{p}^{(1)}$ to compute the Hessian
	matrices of the constraining functions $\Psi_{\ell}(\mathbf{p})$, $\ell = 0, 1, 2, 3$
	(equations \ref{eq:isostatic-constraint-function}, \ref{eq:smootheness-contraint},
	\ref{eq:equality-constraint-basement} and \ref{eq:equality-constraint-moho}), and 
	the Hessian matrix of the misfit function
...
By following the procedures described in the previous section, we applied our method 
...
The parameters $\tilde{\alpha}_{0}$, $\tilde{\alpha}_{1}$, $\tilde{\alpha}_{2}$ and 
$\tilde{\alpha}_{3}$ (equation~\ref{eq:constraint-weights}) used to estimate a parameter 
vector $\mathbf{p}^{(0)}$ without imposing the isostatic constraint have values $0$, $10^{1}$, $10^{1}$ and $10^{2}$, respectively.

Figure \ref{fig:synt-estimated-model-without-isostatic} 
...
\plot{volcanic-margin-grafics-estimated-model-alphas_X_1_1_2}{width=0.6\textwidth}{
	...
	(predicted data) and data produced the model used as initial guess in the 
	inversion (initial guess data).
	All but the isostatic constraint where used. 
	For simplicity, the contour of the prisms forming the 
	interpretation model were omitted.
	The numbers in the legend represent the density contrasts 
	$\Delta\rho^{(\alpha)}$, $\alpha = w, 1, 2, cc, c0, m$. 
	The values are given in kg/m$^{3}$.}
	\label{fig:synt-estimated-model-without-isostatic}
}

\plot{volcanic-margin-grafics-estimated-model-alphas_X_1_1_2_more-known-depths}{width=0.6\textwidth}{
	{Application to synthetic data....}
	\label{fig:synt2-estimated-model-without-isostatic}
}

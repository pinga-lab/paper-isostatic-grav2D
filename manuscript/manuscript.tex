...
the boundaries of juxtaposed sedimentary layers, the relief of basement 
and/or the Mohorovicic discontinuity (or simply Moho), which separates crust and mantle.
...
\citep[e.g.,][]{pilkington-crossley1986b, gallardo-etal2005, condi-etal1999, 
...
reliefs. In their method, \citet{salem-etal2014} estimate the basement
relief by using an iterative approach similar to that
presented by \citet{bott1960} and \citet{cordell-henderson1968}. 
The difference is that, at each iteration, they use the Airy's model 
...
It is reasonable to presume that the method presented by \citet{salem-etal2014} is also
susceptible to these limitations based on its resemblance with that proposed by \citeauthor{bott1960}.
...
In order to produce stable solutions and introduce a priori information,
...
and known depths at some points along the profile. 
We also impose isostatic equilibrium to the estimated model. However,
...
model on a planar surface located at depth, below which we assume that there is no lateral
density variations.
...
must be constant on a planar surface located at depth.
...
We assume that the Crust-Ocean Transition (COT) can be represented by an abrupt and 
vertical interface.
...
In order to obtain stable solutions, we impose the estimated basement and Moho to be 

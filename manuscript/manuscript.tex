...
\documentclass[manuscript,revised,noblind]{geophysics}
...
\footer{macelabastos@gmail.com, vanderlei@on.br}
...
attempts to use isostasy for exploring (but not necessarily reducing) the inherent 
...
Pelotas basin, a passive volcanic margin at the southern of Brazil, 
...
simultaneously.
As pointed out by \citet{silva-etal2014}, \citeauthor{bott1960}'s
...
To produce stable solutions, we use different constraints imposing 
...
that (1) fit the observed data and (2) present different degrees of isostatic 
equilibrium.
...
retrieving the geometry of basement and Moho of a realistic margin model presenting 
an extreme crustal thinning.
...
To define the anomalous mass distribution producing the observed gravity
...
$(x_{i+1}, y_{i+1}, z_{i+1})$. Note that equation~\ref{eq:elements-wii} defines elements 
...
regions. This idea is in agreement with the observation that isostatic equilibrium at 
...
Good starting values are, respectively,
...
Good starting values are $10^{1}$ and $10^{2}$.
These parameters must be the smallest positive numbers producing an estimated model
satisfying the following conditions: (1) the data fit must be good along the entire profile
and (2) the pinnacles must be completely removed or attenuated as much as possible.
...
Good starting values are $10^{2}$ or $10^{3}$.
This parameter must be the largest positive number resulting in an estimated
model satisfying two conditions: 
(1) the lithostatic stress curve must be smoother than that obtained at Step 1 
along the entire profile and 
(2) there must be a good data fit along the entire
...
obtained at Step 2.
This parameter must be set as the largest positive number producing (1) a good data-fit
along the entire profile and (2) an estimated model different from those obtained
at Steps 1 and 2.
...
The geometry of this model is based on a seismic profile crossing the East Greenland margin,
...
Figures~\ref{fig:synt-margin-step3-sgm1}--\ref{fig:synt-margin-step3-sgm18} 
show estimated models $\mathbf{p}^{(3)}$ obtained at
...
The results obtained with the model based on the East Greenland margin illustrate
...
\ref{fig:simple-model-step1}--\ref{fig:simple-model-step2-u0-3} 
show results obtained with a simple model (Table \ref{tab:simple-model}) that 
contains small variations in the lithostatic stress curve and can
...
Notice that, differently for the result obtained with the model based on the East Greenland 
...
for investigating the (1) robustness of our method to the initial approximations used 
in the non-linear inversion, (2) the sensitivity of our method to different combinations 
of constraints and the (3) quantity of a priori information used at basement and Moho.
...
Figures~\ref{fig:pelotas-step3-sgm7}--\ref{fig:pelotas-step3-sgm19} 
show estimated models obtained at
...
equilibrium, except at the region $\approx 150$ km.
...
those used to obtain the models shown in Figures 
\ref{fig:pelotas-step3-sgm7}--\ref{fig:pelotas-step3-sgm19}.
...
masses and produce a gravity high. To compensate this high, the 
...
constraint.
Applications to field data over the Pelotas basin, considered a classical volcanic
...
\tabl{synt-margin-model}{Properties of the synthetic margin model with geometry 
	based on the East Greenland margin. 
...
\plot{B-model-rifted-margin-grafics-estimated-model-alphas_X_1_2_1_2-2pts-dpi300}{width=0.6\textwidth}{
	{Application to synthetic data produced by the model based on the
...
\plot{B-model-rifted-margin-grafics-estimated-model-alphas_3_1_2_1_2-2pts-dpi300}{width=0.6\textwidth}{
	{Application to synthetic data produced by the model based on the
...
\plot{B-model-rifted-margin-grafics-estimated-model-alphas_3_1_2_1_2-sgm_1-2pts-dpi300}{width=0.6\textwidth}{
	{Application to synthetic data produced by the model based on the
...
\plot{B-model-rifted-margin-grafics-estimated-model-alphas_3_1_2_1_2-sgm_11-2pts-dpi300}{width=0.6\textwidth}{
	{Application to synthetic data produced by the model based on the
	East Greenland margin. Estimated model $\mathbf{p}^{(3)}$ 
	obtained at the end of Step 3 (Figure~\ref{fig:flow-diagram}) by
	using $\mathbf{p}^{(2)}$ (Figure~\ref{fig:synt-margin-step2}) as
...
\plot{B-model-rifted-margin-grafics-estimated-model-alphas_3_1_2_1_2-sgm_18-2pts-dpi300}{width=0.6\textwidth}{
	{Application to synthetic data produced by the model based on the
	East Greenland margin. Estimated model $\mathbf{p}^{(3)}$ 
	obtained at the end of Step 3 (Figure~\ref{fig:flow-diagram}) by
	using $\mathbf{p}^{(2)}$ (Figure~\ref{fig:synt-margin-step2}) as
...
\plot{E-model-rifted-margin-grafics-estimated-model-alphas_X_1_2_1_2-dpi300}{width=0.6\textwidth}{
...
\plot{E-model-rifted-margin-grafics-estimated-model-alphas_2_1_2_1_2-dpi300}{width=0.6\textwidth}{
	{Application to synthetic data produced by a simple model. 
...
\plot{E-model-rifted-margin-grafics-estimated-model-alphas_3_1_2_1_2-dpi300}{width=0.6\textwidth}{
	{Application to synthetic data produced by a simple model. 
		Estimated model $\mathbf{p}^{(2)}$ 
		obtained at the end of Step 2 (Figure~\ref{fig:flow-diagram}) by
		using $\tilde{\mu}_{0} = 10^{3}$.
...
\plot{map-line-localization-test-dpi300}{width=0.8\textwidth}{
	...
	$(-27.72^{\circ}, -48.23^{\circ})$ and $(-29.66^{\circ}, -44.94^{\circ})$. 
	The coordinates are referred to the WGS84 datum.}
	\label{fig:pelotas-location}
}

\plot{pelotas-profile-grafics-estimated-model-alphas_X_1_2_1_2-dpi300}{width=0.6\textwidth}{
...
\plot{pelotas-profile-grafics-estimated-model-alphas_2_1_2_1_2-dpi300}{width=0.6\textwidth}{
...
\plot{pelotas-profile-grafics-estimated-model-alphas_2_1_2_1_2-sgm_7-dpi300}{width=0.6\textwidth}{
...
\plot{pelotas-profile-grafics-estimated-model-alphas_2_1_2_1_2-sgm_17dot6-dpi300}{width=0.6\textwidth}{
	{Application to real data on Pelotas basin, Brazil. Estimated model $\mathbf{p}^{(3)}$ 
	obtained at the end of Step 3 (Figure~\ref{fig:flow-diagram}) by
	using $\mathbf{p}^{(2)}$ (Figure~\ref{fig:pelotas-step2}) as
	initial approximation (dashed blue lines) and
...
\plot{pelotas-profile-grafics-estimated-model-alphas_2_1_2_1_2-sgm_19-dpi300}{width=0.6\textwidth}{
	{Application to real data on Pelotas basin, Brazil. Estimated model $\mathbf{p}^{(3)}$ 
	obtained at the end of Step 3 (Figure~\ref{fig:flow-diagram}) by
	using $\mathbf{p}^{(2)}$ (Figure~\ref{fig:pelotas-step2}) as
	initial approximation (dashed blue lines) and
...
\plot{pelotas-profile-grafics-estimated-model-alphas_2_1_2_1_2-sgm_17-T07-dpi300}{width=0.6\textwidth}{
...
The authors are grateful to the editors E. Gasperikova, J. Shragge, X. Li, reviewers 
J. G. Abdeslem, Bob Ferderer and anonymous reviewers for their constructive comments 
and suggestions.
We are deeply indebted to the open-source Python toolkit for geophysical modeling and inversion
Fatiando a Terra \citep{uieda-etal2013}, the open-source libraries Numpy/Scipy 
\citep{jones_etal2001}
for array-based computations, matplotlib \citep{hunter2007} for plotting
and also Jupyter Notebook (\url{https://jupyter.org/}) for putting all together. 
V. C. Oliveira Jr was supported by fellowships from the brazilian research agencies 
CNPq (grant no. 308945/2017-4) and FAPERJ (grant no. E-26/202.729/2018).

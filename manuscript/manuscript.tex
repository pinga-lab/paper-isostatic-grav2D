...
\end{equation}
where $\tilde{\alpha}_{\ell}$ is a positive scalar, $f_{\ell} = \frac{E_{\ell}}{E_{\Phi}}$, 
...
\textbf{PROBLEM HERE:} 
It is important to note that the simple model I has a smooth basement relief
and a non-smooth lithostatic stress curve. 
In this case, the isostatic constraint (equation~\ref{eq:isostatic-constraint-function}) 
does not impose an a priori information that is compatible with
the true model. Consequently, the results obtained by using the isostatic
constraint (Figure~\ref{fig:model-simple-I}) are worse than those show 
in Figure~\ref{fig:model-simple-I-no-vinc-isostatic}.
...
$\tilde{\alpha}_{0} = 0$, $\tilde{\alpha}_{1} = XXXX$, 
$\tilde{\alpha}_{2} = XXXX$ and $\tilde{\alpha}_{3} = XXXX$.
The results obtained with and without the isostatic constraint produced
practically the same data fit. However, there are significant differences
on the lithostatic stress curves and the geometry of the estimated models.
Figure~\ref{fig:model-simple-II} shows that 
the isostatic constraint produces a smooth lithostatic stress
curve, which is in perfect agreement with the a priori information
imposed by the regularizing function $\Psi_{0}(\mathbf{p})$
(equation~\ref{eq:isostatic-constraint-function}).
By comparing the estimated Moho shown in 
Figures~\ref{fig:model-simple-II} and \ref{fig:model-simple-II-no-vinc-isostatic},
we can see that the result obtained by using the isostatic constraint 
is closer to the true one along the entire profile.
On the other hand, the estimated basement is better at the first $\approx 60$ km,
on the region coincident with the abrupt crustal thinning
...
constraint, as shown in Figure~\ref{fig:model-simple-II-no-vinc-isostatic}.
...
%\plot{rifted-margin-model-color}{width=0.6\textwidth}{
...
%\plot{interpretation-model-color}{width=0.6\textwidth}{
...
%\plot{model-simple-I}{width=\columnwidth}{
	{Application to synthetic data produced by the simple model I.
	 ...
	 crust (continental and oceanic) and mantle. The numbers in the legend
	 represent de density values in kg/m$^{3}$.
	 %(Table~\ref{tbl:simple-models}).
...
\plot{model-simple-I-no-vinc-isostatic}{width=0.6\textwidth}{
	...
	{Application to synthetic data produced by the simple model I.
	 The difference between this figure and Figure~\ref{fig:model-simple-I}
...
\plot{model-simple-II}{width=0.6\textwidth}{
	...
	{Application to synthetic data produced by the simple model II.
	 ...
	 by using equation~\ref{eq:lithostatic-stress-densities}. The values are multiplied 
	 by a constant gravity value.
	 The upper panel shows the true gravity disturbance data produced by the simple model I
	 (simulated data), 
	 as well as the data produced by the estimated model (predicted data) and that produced
	 by the model used as initial guess in the inversion (initial guess data).
	 The estimated model was obtained by using all constraints.}
...
\plot{model-simple-II-no-vinc-isostatic}{width=0.6\textwidth}{
	...
	{Application to synthetic data produced by the simple model II.
	 The difference between this figure and Figure~\ref{fig:model-simple-II}
	 is that the estimated model was obtained by using all constraints,
	 except the isostatic constraint.}
}
...
\plot{model-pelotas-synthetic-data}{width=0.6\textwidth}{
	...
	{Application to synthetic data produced by the volcanic margin model.
	 This model is formed by four layers representing: water, sediments $+$
	 SDR, crust (continental and oceanic) and mantle. The numbers in the legend
	 represent de density values in kg/m$^{3}$.
	 % (Table~\ref{tbl:volcanic-margin-model}).
	 The bottom panel shows the estimated model (gray prisms),
	 the true basement and Moho (dashed white lines), the initial basement
	 and Moho used in the inversion (continuous white lines), as well as the 
	 known depths at basement (white triangles) and Moho (white diamonds).

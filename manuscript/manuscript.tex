...
particular parameter vector producing a predicted data
...
are computed by using a central finite difference approximation.

The specific $\alpha_{\ell}$ (equation~\ref{eq:goal-function}) values leading to
reliable estimates of the parameter vector can be very dependent on the 
...
the initial approximation for the parameter vector $\mathbf{p}$.
...
elements $0 > w_{kk} \ge 1$.

Function $\Psi_{0}(\mathbf{p})$ defines the \textit{Isostatic constraint}. 
Notice that, by minimizing this function, the interpreter imposes smoothness on 
the lithostatic stress exerted by the interpretation model on the isostatic 
compensation surface $S_{0}$. Consequently, it forces the interpretation model to be
as close as possible to an isostatic equilibrium. 
The diagonal matrix $\mathbf{W}$ controls the relative amount of isostatic
equilibrium imposed along the profile. By setting $\mathbf{W}$ equal to the identity, the
interpreter imposes the same amount of isostatic equilibrium along the whole profile. By using
different values $0 > w_{kk} \ge 1$, $k = 1, \dots, (N - 1)$, for the elements on 
the diagonal of $\mathbf{W}$, the interpreter can impose different amounts of isostatic
equilibrium along the profile. Elements $w_{kk} \approx 0$ allow abrupt changes on the 
lithostatic stress exerted by the adjacent columns $k$-th and $k+1$-th forming the 

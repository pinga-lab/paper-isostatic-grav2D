...
\plot{rifted-margin-model-color}{width=\columnwidth}{
	...
	represents a water layer with constant density $\rho^{(w)}$. Layer 2
	...
	Layer 3 represents the crust. In this example, it is divided into continental crust, with
	a constant density $\rho^{(cc)}$, and the oceanic crust, with a constant density
	$\rho^{(oc)}$. More complex models can be generated by using a function
	...
	and base at the reference Moho. Below the reference Moho, the reference distribution has the
	same constant density $\rho^{(m)}$ of the mantle.}
	\label{fig:rifted-margin-model}
}

\plot{interpretation-model-color}{width=\columnwidth}{
	{PRECISA ARRUMAR O TEXTO DESSA LEGENDA. 
